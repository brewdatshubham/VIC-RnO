...
with st.expander(f"📊 SCFD 2.0 — Plant Level (VILC + Spend)  ·  {len(df_20):,} rows", expanded=True):
    st.dataframe(df_20, width='stretch', height=380)
...
with st.expander(f"🍺 SCFD 3.0 — Volume  ·  {len(df_vol):,} rows", expanded=True):
    st.dataframe(df_vol, width='stretch', height=380)
...
if cluster_btn:
    if df_vol.empty or "plant" not in df_vol.columns:
        ...
        # ── Mapped: has a valid scfd2 match in the master mapping ──
        df_mapped = mapped[mapped["scfd2_plant"].notna()].copy()

        # ── Unmapped: no scfd2 match in mapping ──
        ...
        # Convert volume to K HL (integer, no decimals)
        df_mapped["volume_khl"] = (df_mapped["volume_hl"] / 1000).round(0).astype(int)

        # Build volume bucket per cluster: "< X K HL" / "X – Y K HL" / "> Y K HL"
        cluster_ranges = (
            df_mapped.groupby("cluster")["volume_khl"]
            .agg(lo="min", hi="max")
            .sort_index()
        )
        def _bucket(cluster_id):
            lo = int(cluster_ranges.loc[cluster_id, "lo"])
            hi = int(cluster_ranges.loc[cluster_id, "hi"])
            prev_clusters = cluster_ranges.index[cluster_ranges.index < cluster_id]
            next_clusters = cluster_ranges.index[cluster_ranges.index > cluster_id]
            if len(prev_clusters) == 0:
                return f"< {hi:,} K HL"
            elif len(next_clusters) == 0:
                return f"> {lo:,} K HL"
            else:
                return f"{lo:,} – {hi:,} K HL"
...
if st.session_state.get("clusters") is not None:
    ...
    st.success(
        ...
        f"**{len(df_unmap):,} plants** excluded (no mapping)"
    )
    ...
    with st.expander("🔵 Cluster Results", expanded=True):
        ...
        st.dataframe(
            df_clust[["scfd3_plant", "scfd2_plant", "canonical_name",
                       "volume_khl", "cluster_label", "volume_bucket"]]
    ...
    if not df_unmap.empty:
        ...
        with st.expander(f"⚠️ Excluded Plants  ·  {len(df_unmap):,} total  |  {unmap_label}", expanded=False):
            for reason, grp in df_unmap.groupby("reason"):
                ...
                st.dataframe(
                    grp[["scfd3_plant", "scfd2_plant", "volume (K HL)"]].sort_values(
                        "volume (K HL)", ascending=False
                    ),
                    width='stretch',
    ...
    for cluster_id in sorted(df_clust["cluster"].unique(), reverse=True):
        ...
        with st.expander(f"📦 {clabel}  ·  {cbucket}  ·  {len(c_plants)} plants", expanded=False):
            ...
            if st.button("Load Analysis", key=f"opp_btn_{cluster_id}", type="primary"):
                with st.spinner(f"Fetching package-level performance for {clabel}…"):
                    try:
                        ...
                        # Clear stale correlation, kpi snapshot and spend snapshot data for this cluster
            ...
            # Package summary — cluster-level aggregation to find worst performing default
            pkg_agg = (
                df_opp.groupby(pkg_col)[perf_col]
                .sum()
                .reset_index()
                .sort_values(perf_col)
                .reset_index(drop=True)
            )
            worst_pkg   = pkg_agg[pkg_col].iloc[0]
            avail_pkgs  = pkg_agg[pkg_col].tolist()
            default_idx = avail_pkgs.index(worst_pkg)
            ...
            # Plant × selected package — one row per plant (base columns)
            df_display = (
            ...
            # Enrich with zone, canonical_name, volume from cluster data
            df_vol_ss  = st.session_state.get("df_vol", pd.DataFrame())
            vol_zone   = (
                df_vol_ss[["plant", "zone"]].drop_duplicates("plant")
                .rename(columns={"plant": "scfd3_plant"})
            ...
            # Per-cluster benchmark controls (only shown for mapped packages)
            if selected_pkg in PACKAGE_KPI_MAP:
                ...
                with col_bm:
                    sel_bm_plant = st.selectbox(
                        "Benchmark Brewery",
                        options=bm_opts,
                        format_func=lambda p: "Auto (best performer)" if p == "auto" else c_plant_display.get(p, p),
                        key=f"bm_plant_{cluster_id}",
                        disabled=manual_bm_kpi is not None,
                    )

            # ── Benchmark analysis (only for packages with a KPI mapping) ──
            if selected_pkg in PACKAGE_KPI_MAP:
                ...
                # KPI snapshot for selected year / month
                ...
                # Correlation across 2022-2025
                if corr_key not in st.session_state:
                    if beer_plants:
                        with st.spinner("Computing correlation (2022–2025)…"):
                            try:
                                df_beer_corr = _fetch_beerometer_for_corr(
                                    beer_plants, target_kpi, zone, country
                                )
                                df_vilc_corr = _fetch_vilc_for_corr(
                                    c_plants, selected_pkg, zone, country
                                )
                ...
                # Map beerometer_plant → scfd2_plant for KPI snapshot
                ...
                # Benchmark determination (per-cluster)
                if sel_bm_plant != "auto":
                    ...
                    # Auto: plant with lowest KPI score (best efficiency) in this cluster
                    valid_kpi = (
                        df_display[kpi_label].dropna()
                        if kpi_label in df_display.columns else pd.Series(dtype=float)
                    )
                    if not valid_kpi.empty:
                        benchmark_idx  = df_display[kpi_label].idxmin()
                ...
                # Manual KPI override takes precedence
                if manual_bm_kpi is not None:
                    benchmark_kpi = manual_bm_kpi

                df_display["benchmark"] = benchmark_name

                # Actual spend per plant for this package (from SpendKPIs AC)
                ...
                if not df_spend_act.empty and "actual_spend" in df_spend_act.columns:
                    df_display = df_display.merge(
                        df_spend_act[["plant", "actual_spend"]],
                        on="plant", how="left",
                    )
                ...
                if benchmark_kpi is not None and not pd.isna(benchmark_kpi) and benchmark_kpi != 0:
                    df_display["kpi delta %"] = (
                        (df_display[kpi_label] - benchmark_kpi) / benchmark_kpi
                    ).round(4)
                    df_display["opp (Mil $)"] = (
                        df_display["spend (Mil $)"] * df_display["kpi delta %"]
                    ).round(3)
                else:
            ...
            color_cols = [c for c in ["performance ($K)", "opp (Mil $)", "perf_corr (r)"] if c in df_show.columns]
            st.dataframe(
            ...
            if selected_pkg in PACKAGE_KPI_MAP:
                ...
                st.caption(
                    f"Benchmark = highest-performing plant · "
                    f"kpi delta % = (plant − benchmark) / benchmark · "
                    f"opp = spend × kpi delta · "
                    f"perf_corr (r): Pearson r vs *{target_kpi}* (AC, 2022–2025, {covered}/{len(c_plants)} plants)"
                )
